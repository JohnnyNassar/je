    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div class="px-5 sm:px-7 py-5 border-b border-gray-100">
                <h1 class="text-xl font-semibold text-gray-900">{{ __('Forgot password?') }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ __('Enter your email. We will send you a reset link.') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('customer.password.email') }}" class="px-5 sm:px-7 py-5 space-y-4">
                @csrf
            ...
            <div class="px-5 sm:px-7 py-4 bg-gray-50 border-t border-gray-100 text-center text-sm">
                <a href="{{ route('customer.login') }}" class="text-brand-600 hover:text-brand-700">← {{ __('Back to sign in') }}</a>
            </div>
        </div>
    </div>
</x-layouts.shop>
