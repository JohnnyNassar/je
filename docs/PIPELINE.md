...
- 🔴 **Security debt — now urgent, not theoretical:** rotate every secret in `joreption.txt` and `git rm --cached` + gitignore it (plaintext creds — incl. server root password + API keys — are committed to the **public** repo); disable root SSH password login. **`/bazar` is live and about to be shared with ~1000 people**, so this stops being a background risk the moment the link goes out. Outstanding since Day 17; declined again on 2026-07-18.
- **Infra:** webhook-triggered auto-deploy (replace the manual SSH git-pull).
- **Dev environment:** the local Git install (`D:\Git`) is **broken** — missing DLLs, every binary exits `0xC0000135`. The 2026-07-18 deploy was pushed via the GitHub API as a workaround. Repair it (`winget install --id Git.Git -e`) before the next deploy.
