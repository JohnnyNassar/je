...
- **The list shows effective access**, not just the role: an **Orders** and a **Cost** icon column, and a role badge that reads **"Administrator · limited"** in amber with a tooltip naming what was withheld when a capability is off. Both render `canViewOrders()` / `canViewCost()` rather than the stored column, since the owner is exempt from the flags and the column would otherwise contradict the truth for the one account that sees everything
...
- ✅ **Imported drafts activated** — _re-checked 2026-09-23: **236 products, all active**, none missing an image, Arabic name, description, category or gallery. The catalogue is no longer a launch blocker._
- **Fix mis-entered variants** — some products carry both choices in a single variant name (e.g. fan #90 = one `white/blue` variant, #91 = one `green`) so the storefront shows no chooser; split each into separate variants (or a Colour option + Build combinations) and divide the stock. _2026-09-23: still **12 products** — #90, #91, #100, #116, #134, #222, #227, #272, #277, #295, #313, #314 (the count keeps growing as products are added, so it is worth a pass before launch). Their stock is now editable from the list via the **Stock by variation** action._
