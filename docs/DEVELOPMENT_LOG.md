...
### Several certificates per booking — and two bugs it exposed
The owner asked for more than one document per booking (a licence and its renewal, a certificate per product line). The schema already allowed it — `bazaar_booking_documents` was always a one-to-many keyed by `kind` — so only the form and controller had assumed a single file. Both fields now take **up to 4 files each**, validated per file (`work_certificate.*`), and the confirmation lists everything received.

**Bug 1 — the server was quietly smaller than the form.** Checking limits before widening the form found production at **`upload_max_filesize = 2M`** while validation accepted **5 MB**. PHP discards an oversized upload *before* validation runs, so a food vendor photographing a certificate on a phone (routinely 3–5 MB) would have submitted, had the file vanish, and been told *"a health certificate is required"* with no explanation — then retried the same file and hit the same wall. Live since the certificates shipped hours earlier. Local PHP allows 40 MB, which is exactly why no test caught it. Production raised to **8 MB per file / 64 MB per request**, nginx to 80 MB, verified against `php-fpm8.3 -i` (the CLI loads a *different* ini and reports the wrong numbers). A test now asserts the runtime limits are at least as large as the form claims to accept.

**Bug 2 — `multiple` does not mean "add".** A file input **replaces its entire `FileList` on every pick**, so choosing a second certificate silently dropped the first; the field looked like it accepted several but only kept the last batch. Fixed by holding the selection in the page and writing it back to the input through a **`DataTransfer`**, so the form still posts real files and the server needed no change. Each file shows with its size and a remove button, duplicates (same name *and* size) are ignored, and the cap warns rather than truncating silently. The browser tooling could not reach this machine, so the accumulate logic was extracted and run under **Node against 11 cases** (second pick appends, cap holds, duplicates ignored, same-name-different-size kept, kinds independent) — that verifies the algorithm, not the DOM wiring, which was flagged to the owner for a manual click-through.

### ⚠️ Found: file uploads are not backed up at all
Answering "where do the uploads go?" turned up that `/usr/local/bin/joreption-backup.sh` is **only** `mariadb-dump | gzip`. Nothing under `storage/app` is backed up — so a lost server loses every vendor certificate *and* the **140 MB of product images** and 22 MB of Cover-logo originals that have never been covered either. The database would restore perfectly with every image and document a broken link. 140 GB free, so this is not a space problem — the script was simply only ever written for the database. Raised with the owner; see `PIPELINE.md`.
...
- **Validation limits must never exceed the server's own limits.** PHP silently drops an upload larger than `upload_max_filesize` *before* any framework code runs, so the app reports "file missing" rather than "file too big" and the user has no idea what to change. Keep `upload_max_filesize` above the per-file rule and `post_max_size` (plus nginx `client_max_body_size`) above the worst-case total, and assert it in a test.
- **Check FPM's ini, not the CLI's.** `php -i` on the server reads `/etc/php/8.3/cli/php.ini` and reported the *old* upload limits after `/etc/php/8.3/fpm/php.ini` had already been changed. `php-fpm8.3 -i` is the one that reflects what the website actually enforces.
- **`<input type="file" multiple>` replaces, it does not accumulate.** `multiple` only permits selecting several files *in one gesture*; the next pick discards the previous `FileList`. To let a user build a set across several picks, keep the list in JS and assign it back via `new DataTransfer()` — the form then submits normally with no server-side change.
- **A database backup is not a backup.** The daily job dumped MariaDB and nothing else, so every uploaded image and certificate sat on a single disk with no copy anywhere. Worth checking what a backup script actually covers before trusting it — the gap only shows up the day you need it.
